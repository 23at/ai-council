...
When you send a message, a coordinator model first decides which agents would realistically reply (sometimes just one, sometimes two, rarely all three). The agents then respond sequentially so each one can "read" what the others said. Every agent has full conversation history for context.
